...
def openApps(ints, message):
    ...
    apps = {"photo": "ms-photos:", "camera": "microsoft.windows.camera:", "word": "winword", "powerpoint": "powerpnt",
    ...
    for ent in doc.ents:
        ...
        if (entity == 'GPE'):
            ...
            if data["cod"] != "404":
                ...
                now = datetime.datetime.now()
                content = "Today is {day}th, {month}, {year}  \nAverage temperature is {temp}°C\nFeels like {feelslike}°C\nMin temperature is {min}°C\nMax temperature is {max}°C\nHumidity is {humidity}%\nDiscription: {des}".format(
                    day=now.day, month=now.month, year=now.year,
                    temp=current_temperature, feelslike=feels_like, min=min_temp, max=max_temp,
                    humidity=current_humidity, des=weather_description)
